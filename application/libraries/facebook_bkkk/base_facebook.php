...
abstract class BaseFacebook
{
  ...
  public function getUser() {
  	log_message("debug","inside getting userinfo");
    if ($this->user !== null) {
  ...
  protected function getUserFromAvailableData() {
  	log_message("debug","inside getting userinfo from form  avas");
    // if a signed request is supplied, then it solely determines
    // who the user is.
    $signed_request = $this->getSignedRequest();
    if ($signed_request) {
    	log_message("debug","request signed");
      if (array_key_exists('user_id', $signed_request)) {
        ...
        $this->setPersistentData('user_id,email', $signed_request['user_id']);
    ...
    $user = $this->getPersistentData('user_id,email', $default = 0);
    ...
    if ($access_token &&
      ...
      if ($user) {
        $this->setPersistentData('user_id', $user);
      	...
      	log_message("debug","elsseee");
        $this->clearAllPersistentData();
      }
    }
    log_message("debug","retuning>>>>".print_r($user,true));
  ...
  public function getLoginUrl($params=array()) {
    ...
    return $this->getUrl(
      ...
      array_merge(array(
                    'client_id' => $this->getAppId(),
                    'redirect_uri' =>'http://cyclingcities.local', // possibly overwritten
                    'state' => $this->state),
                  $params));
  }
