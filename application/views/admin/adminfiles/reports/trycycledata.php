<div class="row">
  ...
  function resetdates()
  {
	  ...
	  window.location = "<?php echo site_url('index.php/admins/reports/trycycledata');?>";
  }
  </script>

    <div class="box">
      <div class="box-header">
        <h3 class="box-title">Try Cycle Data</h3>
        <form method="post" enctype="multipart/form-data" action="<?php echo site_url('index.php/admins/reports/searchtrycycledata');?>" onsubmit="return checkstartdate();">
                    <!-- text input -->
						...
						foreach ($result as $row)
						  { ?>
							  <tr>
								...
								<td><?php if(count($this->post_cycle_model->getbrandname($row['brand'])))
										{
										  echo $this->post_cycle_model->getbrandname($row['brand']);
										}
									 ?>
								</td> 
								<td><?php if(count($this->post_cycle_model->getmodelname($row['model'])))
										{
										   echo($this->post_cycle_model->getmodelname($row['model'])); 
										}
									 ?>
								</td>
