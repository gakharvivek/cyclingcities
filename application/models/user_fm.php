...
class user_fm extends CI_Model
{
	...
	function addfacebookuser( $data )
	{
		...
		else
		{
			...
			$this->db->insert('users_front', $datainsert);
			$RegistrationsId=$this->db->insert_id();
			$data['frontloggedin'] = TRUE;
			log_message('debug',"valuess after data set".print_r($data,true));
			$this->session->set_userdata($data);
			//$_SESSION['fbuserid'] = $data['id'];
			log_message('debug',"insert sessions".print_r($_SESSION,true));
			return $RegistrationsId;
		}
	...
	{      
		...
		log_message("debug","value of user verification>>>".print_r($user,true));
		if(count($user)>0)
		{
			$fname=ucfirst($user[0]["firstname"]);
			$lname=ucfirst($user[0]["lastname"]);
			$username = $user[0]['email'];
			//$password = $user[0]['password'];
			$address = $user[0]['email'];
			log_message("debug","user first>>>".$fname);

			$from = $this->config->item('fromemailid');
			log_message("debug","mail from>>>".$from);
